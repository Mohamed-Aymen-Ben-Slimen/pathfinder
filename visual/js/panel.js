...
var Panel = {
    ...
    getFinder: function() {
        ...
        switch (selected_header) {
            ...
            case 'bestfirst_with_minimum_turns_header':
                allowDiagonal = typeof $('#bestfirst_with_minimum_turns_section ' +
                    '.allow_diagonal:checked').val() !== 'undefined';
                biDirectional = typeof $('#bestfirst_with_minimum_turns_section ' +
                    '.bi-directional:checked').val() !== 'undefined';
                dontCrossCorners = typeof $('#bestfirst_with_minimum_turns_section ' +
                    '.dont_cross_corners:checked').val() !== 'undefined';

                /* parseInt returns NaN (which is falsy) if the string can't be parsed */
                weight = parseInt($('#bestfirst_with_minimum_turns_section .spinner').val()) || 1;
                weight = weight >= 1 ? weight : 1; /* if negative or 0, use 1 */

                heuristic = $('input[name=bestfirst_with_minimum_turns_heuristic]:checked').val();
                if (biDirectional) {
                    finder = new PF.BiAStarFinder({
                        allowDiagonal: allowDiagonal,
                        dontCrossCorners: dontCrossCorners,
                    ...
                    console.log(PF);
                    console.log(PF.BestFirstFinder);
                    console.log(PF.BestFirstFinderWithMinimumTurns);
                    finder = new PF.BestFirstFinderWithMinimumTurns({
                        allowDiagonal: allowDiagonal,
                        dontCrossCorners: dontCrossCorners,
                        heuristic: PF.Heuristic[heuristic],
                        weight: weight
